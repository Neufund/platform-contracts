...
contract(
  "FeeDisbursal",
  ([_, masterManager, disburser, disburser2, contractMock, ...investors]) => {
    let universe;
    ...
    describe("specific tests", () => {
      ...
      async function disburseNeumark(sender, amount, expIndex) {
        ...
        const tx = await neumark.transfer["address,uint256,bytes"](
          feeDisbursal.address,
          amount,
          0,
          {
      ...
      describe("test FeeDisbursalController", () => {
        ...
        async function contractClaimAllowed(claimf, interfaceHash) {
          let allowed = await claimf(ZERO_ADDRESS, ZERO_ADDRESS, contractMock);
          expect(allowed).to.be.false;
          await universe.setCollectionInterface(interfaceHash, contractMock, true, {
            from: masterManager,
          });
          allowed = await claimf(ZERO_ADDRESS, ZERO_ADDRESS, contractMock);
          expect(allowed).to.be.true;
          await universe.setCollectionInterface(interfaceHash, contractMock, false, {
            from: masterManager,
          });
        }
        ...
        it("should reject for contracts that can disburse", async () => {
          await contractClaimAllowed(
            feeDisbursalController.onReject,
            knownInterfaces.commitmentInterface,
          );
          await contractClaimAllowed(
            feeDisbursalController.onReject,
      ...
      it("should retrieve non claimable disbursals", async () => {
        ...
        // create two entries for ethertoken
        await disburseEtherToken(disburser, Q18.mul(40));
        const one = await feeDisbursal.getNonClaimableDisbursals(
          etherToken.address,
          neumark.address,
        );
        ...
        await disburseEtherToken(disburser2, Q18.mul(60));
        const two = await feeDisbursal.getNonClaimableDisbursals(
          etherToken.address,
          neumark.address,
        );
      ...
      it("should be able to retreive multiple claimables", async () => {
        ...
        const expectedEtherClaimables0 = [
          [0, 0, 0, 0],
        ...
        const expectedEtherClaimables1 = [
          [0, 0, 0, 0],
      ...
      it("should accept multiple tokens for one pro-rata", async () => {
        const paymentTokens = [euroToken.address, etherToken.address];
        await expect(
          feeDisbursal.acceptMultipleByToken(paymentTokens, neumark.address, {
            from: investors[0],
          }),
      ...
      it("should disburse fully with various roundings in pro rata calculations", async () => {
        ...
        await advanceSnapshotId(neumark);
        const [snapshotId] = await feeDisbursal.getDisbursal(
          etherToken.address,
          neumark.address,
          0,
        );
      ...
      describe("access control", () => {
        ...
        it("should not allow setting a controller from random address", async () => {
          ...
          const newController = await FeeDisbursalController.new(universe.address);
          await expect(
            feeDisbursal.changeFeeDisbursalController(newController.address, {
              from: investors[3],
            }),
