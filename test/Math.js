...
// const divRound = (v, d) => d.dividedToIntegerBy(2).plus(v).dividedToIntegerBy(d);
const Q18 = new BigNumber(10).pow(18);
const B56 = new BigNumber(2).pow(56);
const B255 = new BigNumber(2).pow(255);
...
contract("Math", () => {
  ...
  it("should absDiff", async () => {
    ...
    // two's complement
    const a = new BigNumber(2).pow(256).minus(5); // uint(-5)
    const b = new BigNumber(2).pow(256).minus(3); // uint(-3)
  ...
  it("should compute fractions", async () => {
    ...
    const ethRate = Q18.times(345.7651);
    const etherAmount = Q18.times(76182.187189893);
    // below is a result of calculation on decimal digits
    const expectedEur = new BigNumber("26341141571932080000000000");
    ...
    diff = resultOneThird
      .minus(expectedOneThird)
      .abs()
      .div(Q18);
    expect(diff.e).to.be.lte(acceptedPrecissionLoss);

    // do not lose precision on binary expansions
    const bAmountMul = new BigNumber("1110101010111001100110101101110001", 2);
    const bAmount = B56.times(bAmountMul);
    const bRateMul = new BigNumber("10010111101011111010101", 2);
    const bRate = B56.times(bRateMul);
    const bResult = await math._decimalFraction(bAmount, bRate);
    // how to get binary decimal from Mathematica directly?
    const bExpectedResult = new BigNumber(
      ...
      2,
    ).div(new BigNumber("110111100000101101101011001110100111011001", 2));
  ...
  it("should do proportions", async () => {
    ...
    const total = Q18.times(9398192.88127981);
    const spent = Q18.times(718921.19818982);
    const lockedNeumark = Q18.times(39889217377);
    const result = await math._proportion(lockedNeumark, spent, total);
    const expectedResult = new BigNumber("3051352990280604000000000000");
    const diff = result
      .minus(expectedResult)
      .abs()
      .div(Q18);
