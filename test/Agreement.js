...
contract("Agreement", ([_, platformOperatorRepresentative, signer1, signer2]) => {
  let agreement;
  let accessPolicy;
  let forkArbiter;
  ...
  beforeEach(async () => {
    ...
    agreement = await TestAgreement.new(accessPolicy.address, forkArbiter.address);
  ...
  it("should reject to amend from unknown account", async () => {
    const agreementUri =
      "txhash:0xa5b600fec63223ba2e7be527b90a2261c818c5491fce11852b2f96a472507cb7";
    await expect(agreement.amendAgreement(agreementUri, { from: signer2 })).to.be.rejectedWith(
      EvmError,
    );
  });
  ...
  it("should amend agreement twice", async () => {
    ...
    const txBlock2 = await promisify(web3.eth.getBlock)(tx2.receipt.blockNumber);
  ...
  it("should reject to sign when no agreement", async () => {
    await expect(agreement.signMeUp({ from: signer1 })).to.be.rejectedWith(EvmError);
  });

  it("should reject current agreement when no agreement", async () => {
    await expect(agreement.currentAgreement.call()).to.be.rejectedWith(EvmError);
  });
