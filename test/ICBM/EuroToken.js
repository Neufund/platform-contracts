...
contract("ICBMEuroToken", ([_, depositManager, other, broker, reclaimer, ...investors]) => {
  let rbap;
  let euroToken;
  ...
  describe("specific tests", () => {
    ...
    it("should not transfer from not allowed", async () => {
      await expect(euroToken.transfer(investors[1], 0, { from: investors[0] })).to.be.rejectedWith(
        EvmError,
      );
    });
  ...
  describe("migration tests", () => {
    ...
    beforeEach(async () => {
      testEuroTokenMigrationTarget = await TestICBMEuroTokenMigrationTarget.new(euroToken.address);
    });
    ...
    it("should migrate", async () => {
      ...
      const tx = await euroToken.enableMigration(testEuroTokenMigrationTarget.address, {
        from: depositManager,
      });
      ...
      expectEuroTokenOwnerMigratedEvent(migrateTx, investors[1], initialBalance);
      ...
      const migratedBalance = await testEuroTokenMigrationTarget.balanceOf(investors[1]);
      expect(migratedBalance).to.be.bignumber.eq(initialBalance);
      // check if EURT are at investor's disposal
      await testEuroTokenMigrationTarget.transfer(investors[2], initialBalance, {
        from: investors[1],
      });
    });
    ...
    it("should reject migration without 'to' permission", async () => {
      ...
      await expect(euroToken.migrate({ from: investors[1] })).to.be.rejectedWith(EvmError);
    });
