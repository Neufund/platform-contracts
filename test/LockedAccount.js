/* eslint-disable */
// todo: enable eslint when tests are done
  ...
  ([_, admin, investor, investor2, otherMigrationSource, operatorWallet]) => {
    ...
    describe("EtherToken", () => {
      async function deployEtherToken() {
        icbmAssetToken = await ICBMEtherToken.new(accessPolicy.address);
      ...
      beforeEach(async () => {
        ...
        await deployLockedAccount(assetToken);
      });
    ...
    describe("EuroToken", () => {
      async function applyTransferPermissions(permissions) {
        for (const p of permissions) {
          switch (p.side) {
            case "from":
              await icbmAssetToken.setAllowedTransferFrom(p.address, true, {
                from: admin,
              });
              break;
            default:
              await icbmAssetToken.setAllowedTransferTo(p.address, true, {
                from: admin,
              });
              break;
          }
      ...
      async function deployEuroToken() {
        icbmAssetToken = await ICBMEuroToken.new(accessPolicy.address);
      ...
      beforeEach(async () => {
        ...
        await applyTransferPermissions([
          { side: "from", address: icbmAssetToken.address },
          { side: "to", address: icbmAssetToken.address },
    ...
    async function deployICBMLockedAccount(
      token,
      feeDisbursalAddress,
      lockPeriod,
      ...
      icbmLockedAccount = await ICBMLockedAccount.new(
      ...
      controller = await TestICBMLockedAccountController.new(icbmLockedAccount.address);
      if (!leaveUnlocked) {
        await icbmLockedAccount.setController(controller.address, {
          from: admin,
        });
      }
    ...
    async function deployLockedAccount(token) {
