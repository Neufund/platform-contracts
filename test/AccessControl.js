...
contract(
  ...
  ([accessController, owner1, owner2, newAccessController, ...accounts]) => {
    ...
    function expectAccessPolicyChangedEvent(tx, controller, oldPolicy, newPolicy) {
    ...
    async function expectRepeatPermissionDoesNothing(state) {
      await accessPolicy.setUserRole(owner2, exampleRole, accessControlled.address, state);
      const tx = await accessPolicy.setUserRole(
        owner2,
        exampleRole,
        accessControlled.address,
    ...
    it("should disallow on unset cascade", async () => {
      ...
      await accessPolicy.setUserRole(owner2, exampleRole, accessControlled.address, TriState.Allow);
      const disallowOnAllUnset = await accessPolicy.allowed.call("0x0", "", "0x0", "");
      expect(disallowOnAllUnset).to.be.false;
      const disallowOnSubjectSet = await accessPolicy.allowed.call(owner2, "", "0x0", "");
      expect(disallowOnSubjectSet).to.be.false;
      const disallowOnRoleSet = await accessPolicy.allowed.call(owner2, exampleRole, "0x0", "");
    ...
    it("should change policy on contract", async () => {
      ...
      await accessPolicy.setUserRole(owner1, exampleRole, accessControlled.address, TriState.Allow);
      ...
      await expect(accessControlled.someFunction({ from: owner1 })).to.be.rejectedWith(EvmError);
    ...
    it("should transfer access controller on contract to new account", async () => {
      ...
      await expect(
        accessControlled.setAccessPolicy(accessPolicy.address, accessController, {
          from: accessController,
        }),
      ).to.be.rejectedWith(EvmError);
      // new access controller controls policy on contract
      await accessControlled.setAccessPolicy(accessPolicy.address, newAccessController, {
        from: newAccessController,
      });
      // however new access controller does not control permissions on policy
      await expect(
        accessPolicy.setUserRole(owner1, exampleRole, accessControlled.address, TriState.Deny, {
          from: newAccessController,
        }),
      ).to.be.rejectedWith(EvmError);
      ...
      await accessControlled.setAccessPolicy(accessPolicy.address, accessController, {
        from: accessController,
      });
    });
    ...
    it("subject EVERYONE should give access to any msg.sender", async () => {
      ...
      const anotherAccessControlled = await TestAccessControl.new(accessPolicy.address);
      await expect(anotherAccessControlled.someFunction({ from: owner1 })).to.be.rejectedWith(
        EvmError,
      );
    });
    ...
    it("object GLOBAL should give access to all contracts", async () => {
      const anotherAccessControlled = await TestAccessControl.new(accessPolicy.address);
      await accessPolicy.setUserRole(owner1, exampleRole, GLOBAL, TriState.Allow);
      await accessControlled.someFunction({ from: owner1 });
      await anotherAccessControlled.someFunction({ from: owner1 });
      // no access for other subjects however
      await expect(accessControlled.someFunction({ from: newAccessController })).to.be.rejectedWith(
        EvmError,
      );
    });
    ...
    it("subject EVERYONE adn object GLOBAL should give access to any msg.sender for any contract", async () => {
      const anotherAccessControlled = await TestAccessControl.new(accessPolicy.address);
      await accessPolicy.setUserRole(EVERYONE, exampleRole, GLOBAL, TriState.Allow);
      await accessControlled.someFunction({ from: owner1 });
      await accessControlled.someFunction({ from: owner2 });
      await anotherAccessControlled.someFunction({ from: owner1 });
    ...
    async function triggerCascade(subject, object, rootSubject, rootObject) {
      await accessPolicy.setUserRole(subject, exampleRole, object, TriState.Allow);
      const allowed = await accessPolicy.allowed.call(rootSubject, exampleRole, rootObject, "");
      expect(allowed).to.be.true;
      await subjectIsListed(object, exampleRole, subject);
      await accessPolicy.setUserRole(subject, exampleRole, object, TriState.Unset);
    ...
    it("should allow access cascading up", async () => {
      ...
      // set everyone local
      await triggerCascade(EVERYONE, accessControlled.address, owner1, accessControlled.address);
      // set owner global
      await triggerCascade(owner1, GLOBAL, owner1, accessControlled.address);
      // set owner local
      await triggerCascade(owner1, accessControlled.address, owner1, accessControlled.address);
    });
    ...
    it("should deny when at the bottom of cascade if all allowed", async () => {
      ...
      await accessPolicy.setUserRole(EVERYONE, exampleRole, GLOBAL, TriState.Deny);
      allowed = await accessPolicy.allowed.call(owner1, exampleRole, accessControlled.address, "");
      expect(allowed).to.be.true;
      ...
      await accessPolicy.setUserRole(owner1, exampleRole, GLOBAL, TriState.Deny);
      allowed = await accessPolicy.allowed.call(owner1, exampleRole, accessControlled.address, "");
      expect(allowed).to.be.true;

      await accessPolicy.setUserRole(owner1, exampleRole, accessControlled.address, TriState.Deny);
      allowed = await accessPolicy.allowed.call(owner1, exampleRole, accessControlled.address, "");
      expect(allowed).to.be.false;
    });
    ...
    describe("enumerating subjects", () => {
      ...
      it("should enumerate default ACCESS_CONTROL permissions", async () => {
        const globSubs = await accessPolicy.getUsers(GLOBAL, roles.accessController);
        expect(globSubs).to.have.same.members([accessController]);
        const locSubs = await accessPolicy.getUsers(accessPolicy.address, roles.accessController);
      ...
      it("should remove last enumerated subject", async () => {
        ...
        await accessPolicy.setUserRole(deletedAccount, exampleRole, GLOBAL, TriState.Unset);
      ...
      it("should remove second enumerated subject", async () => {
        ...
        await accessPolicy.setUserRole(deletedAccount[0], exampleRole, GLOBAL, TriState.Unset);
        expect(subjects).to.have.same.members(accountsCopy);
      });

      it("should not add duplicate when setting permission again", async () => {
        const subjects = await accessPolicy.getUsers(GLOBAL, exampleRole);
        await accessPolicy.setUserRole(accounts[0], exampleRole, GLOBAL, TriState.Deny);
      ...
      it("should add again after Unset", async () => {
        ...
        await accessPolicy.setUserRole(subjects[0], exampleRole, GLOBAL, TriState.Unset);
        // add again
        await accessPolicy.setUserRole(subjects[0], exampleRole, GLOBAL, TriState.Deny);
        expect(subjects).to.have.same.members(accounts);
      });
    });
    ...
    describe("self policing", () => {
      ...
      it("should reject changing permissions from invalid account", async () => {
        ...
        await expect(
          accessPolicy.setUserRole(owner1, exampleRole, accessControlled.address, TriState.Allow, {
            from: newAccessController,
          }),
        ).to.be.rejectedWith(EvmError);

        await accessPolicy.setUserRoles([owner1], [exampleRole], [GLOBAL], [TriState.Allow], {
          from: accessController,
        });
        await expect(
          accessPolicy.setUserRole([owner1], [exampleRole], [GLOBAL], [TriState.Allow], {
            from: newAccessController,
          }),
        ).to.be.rejectedWith(EvmError);
      });
