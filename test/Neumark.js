...
contract(
  ...
  ([deployer, other, platformRepresentative, transferAdmin, issuer1, issuer2, ...accounts]) => {
    ...
    async function initNeumarkBalance(initialBalanceNmk, distributeTo) {
      ...
      await neumark.issueForEuro(initialBalanceNmk.times(6.5).round(), {
        from: issuer1,
      });
      const balance = await neumark.balanceOf.call(issuer1);
      await neumark.burn.uint256(balance.minus(initialBalanceNmk), {
        from: issuer1,
      });
      // every ulp counts
    ...
    describe("general tests", () => {
      ...
      it("should have curve parameters", async () => {
        expect(await neumark.neumarkCap.call()).to.be.bignumber.eq(NMK_DECIMALS.times(1500000000));
        expect(await neumark.initialRewardFraction.call()).to.be.bignumber.eq(
          NMK_DECIMALS.times(6.5),
        );
      });
      ...
      it("should issue Neumarks", async () => {
        ...
        const expectedr1EUR = EUR_DECIMALS.times(100);
        const r1 = await neumark.issueForEuro(expectedr1EUR, {
          from: issuer1,
        });
        await prettyPrintGasCost("Issue", r1);
        const expectedr1NMK = new BigNumber("649999859166687009257");
        const r1NMK = eventValue(r1, "LogNeumarksIssued", "neumarkUlps");
        expect(r1NMK.minus(expectedr1NMK).abs()).to.be.bignumber.lessThan(2);
        ...
        const expectedr2EUR = EUR_DECIMALS.times(900);
        const r2 = await neumark.issueForEuro(expectedr2EUR, {
          from: issuer2,
        });
        const expectedr2NMK = new BigNumber("5849986057520322227964");
        const expectedTotalNMK = new BigNumber("6499985916687009237221");
        const r2NMK = eventValue(r2, "LogNeumarksIssued", "neumarkUlps");
        expect(r2NMK.minus(expectedr2NMK).abs()).to.be.bignumber.lessThan(2);
      ...
      it("should issue and then burn Neumarks", async () => {
        ...
        const euroUlps = EUR_DECIMALS.times(1000000);
        ...
        const toBurnUlps = NMK_DECIMALS.times(toBurn);
        ...
        ).to.be.bignumber.eq(neumarks.minus(toBurn));
      ...
      async function expectIncrementalInverseWalk(expectedPoints) {
        ...
        const initialEurUlps = new BigNumber("100000000000").times(EUR_DECIMALS);
        ...
        for (const [e, n] of expectedPoints) {
          ...
          if (burnNmk.gt(0)) {
            ...
            if (controlCurveNmk.minus(newTotalNmk).abs().gt(0)) {
              console.log(`control nmk do not equal totalNmk ${controlCurveNmk.minus(newTotalNmk).toNumber()}`)
            } */
          }
        }
      ...
      it("should burn with range with low gas cost", async () => {
        ...
        const burnNmk = new BigNumber("3.21504457765812047556165399769811884e7").minus(
          afterBurnNmk,
        );
        ...
        const actualInverseEurDeltaUlps = totalEuroUlps.minus(await neumark.totalEuroUlps.call());
        expect(
          actualInverseEurDeltaUlps.minus(expectedInverseEurDeltaUlps).abs(),
      ...
      it("should transfer Neumarks", async () => {
        await neumark.issueForEuro(EUR_DECIMALS.times(100), { from: issuer1 });
      ...
      it("should accept agreement on issue Neumarks", async () => {
        const tx = await neumark.issueForEuro(EUR_DECIMALS.times(100), {
          from: issuer1,
        });
    ...
    describe("IBasicToken tests", () => {
      const initialBalanceNmk = NMK_DECIMALS.times(1128192.2791827).round();
    ...
    describe("IERC20Allowance tests", () => {
      const initialBalanceNmk = NMK_DECIMALS.times(91279837.398827).round();
      const getToken = () => neumark;
