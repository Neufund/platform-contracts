...
contract("ETOCommitment", ([, admin, company, nominee, ...investors]) => {
  ...
  describe("MockETOCommitment tests", () => {
    ...
    it("should move time to right before state transition", async () => {
      ...
      // move to right before startDate via helper and wait
      await etoCommitment._shiftToBeforeNextState(10);
      await increaseTime(15);
      const tx = await etoCommitment.handleStateTransitions();
      expectLogStateTransition(tx, CommitmentState.Setup, CommitmentState.Whitelist, /* timestamp */ "ignore");
      ...
      expectLogStateTransition(tx2, CommitmentState.Whitelist, CommitmentState.Public, /*timestamp:*/ "ignore" );

      // while public, invest in ETO, so next state will be Claim ?

      // await investAmount(investors[0], Q18.mul(23987.288912), "EUR");
      // let currentState = await etoCommitment.state();
      // console.log('after Invest ', currentState.toNumber() );

      // await etoCommitment._shiftToBeforeNextState(10);
      // await increaseTime(15);
      // const tx3 = await etoCommitment.handleStateTransitions();

      // currentState = await etoCommitment.state();
      // console.log('after call ', currentState.toNumber() );
      // expect(await etoCommitment.state()).to.be.bignumber.gt(CommitmentState.Public);
      // expect(await etoCommitment.state()).to.be.bignumber.eq(CommitmentState.Claim);
      // expectLogStateTransition(tx3, CommitmentState.Public, CommitmentState.Signing, /*timestamp:*/ "ignore" );
    })
